...
@Component({
  ...
  styleUrls: ['./analyse-video.component.css'],
  animations: [
    trigger('fadeInOut', [
      transition(':enter', [
...
export class AnalyseVideoComponent {
  ...
  // Update detected objects based on current time
  private updateDetectedObjects(): void {
    // Simulate object detection at different timestamps
    const timestamp = Math.floor(this.currentTime);
    this.detectedObjects = [
      { type: 'Plastic Bottle', confidence: 0.95, timestamp },
      { type: 'Fishing Net', confidence: 0.85, timestamp },
      { type: 'Debris', confidence: 0.75, timestamp }
    ];
  }
